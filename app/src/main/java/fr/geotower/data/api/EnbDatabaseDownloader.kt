...
/**
 * Telechargement de `geotower_fr_enb.db` (identifiants reseau eNB/gNB, donnee du partenaire
 * eNB-Analytics redistribuee par le serveur GeoTower).
 *
 * Meme chaine de confiance que la base radio : l'URL et l'empreinte SHA-256 viennent du
 * **manifeste signe**, jamais d'une URL en dur cote reponse ; le fichier est verifie (taille,
...
object EnbDatabaseDownloader {
    ...
    private const val DB_NAME = EnbDatabaseValidator.DB_NAME
            ...
            try {
                ...
                response.use { safeResponse ->
                    if (!safeResponse.isSuccessful) {
                        AppLogger.w(TAG, "eNB database download HTTP ${safeResponse.code}")
                        return@withContext false
                    }
                    ...
                    if (expectedContentLength > maxAllowedBytes) {
                        AppLogger.w(TAG, "Remote eNB database content length exceeds hard limit")
                        return@withContext false
                    }
                        ...
                        FileOutputStream(tempFile).use { outputStream ->
                            ...
                            while (bytes >= 0) {
                                ...
                                if (bytesCopied > maxAllowedBytes) {
                                    AppLogger.w(TAG, "Remote eNB database download exceeded hard limit")
                                    throw IllegalStateException("enb_database_download_too_large")
                                }
                    ...
                    if (!hasExpectedDownloadIntegrity(tempFile, expectedContentLength, expectedSizeBytes, expectedSha256)) {
                        AppLogger.w(TAG, "Downloaded eNB database integrity mismatch")
                        tempFile.delete()
                        return@withContext false
                    }
                }

                val validation = EnbDatabaseValidator.validateDatabaseFile(tempFile)
                if (!validation.isValid) {
                    AppLogger.w(TAG, "Downloaded eNB database validation failed: ${validation.reason}")
                    tempFile.delete()
                    return@withContext false
                }
    ...
    internal fun isOfficialEnbDatabaseDownloadUrl(url: String): Boolean {
    ...
    internal fun isValidRemoteEnbDatabaseInfo(
        filename: String,
        sizeBytes: Long,
        sha256: String,
        schemaVersion: Int?,
        ...
        if (maxAllowedEnbDatabaseDownloadBytes(sizeBytes) == null || !isValidSha256(sha256)) {
            return false
        }

        if (schemaVersion != null && schemaVersion != EnbDatabaseValidator.EXPECTED_SCHEMA_VERSION) {
            return false
        }
        ...
        if (
            countryCode != null &&
            !countryCode.equals(
                EnbDatabaseValidator.EXPECTED_COUNTRY_CODE,
                ignoreCase = true
            )
        ) {
            return false
    ...
    /**
     * Plafond dur : la base eNB pese quelques Mo. On garde une marge large (128 Mio) pour ne pas
     * bloquer une croissance normale du parc, tout en refusant une reponse aberrante.
     */
    private fun maxAllowedEnbDatabaseDownloadBytes(sizeBytes: Long): Long? {
        if (sizeBytes <= 0L || sizeBytes > 128L * 1024L * 1024L) return null
