...
fun SmallWavyLoader(
    ...
    Canvas(modifier = modifier) {
        ...
        val height = size.height
        val center = Offset(width / 2, height / 2)
        val radius = (width / 2) * 0.85f // On laisse un peu plus de marge pour éviter de couper les bords

        // Calibrage spécifique pour les petites tailles
        val waveAmplitude = 0.8.dp.toPx() // Vagues plus douces
        val waveFrequency = 8 // Moins de vagues pour ne pas surcharger l'affichage
        ...
        rotate(degrees = rotation.value) {
            ...
            drawPath(
                path = segmentPath,
                color = color,
                // Trait plus fin adapté aux petits affichages
