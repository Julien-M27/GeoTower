...
fun RadioDatabaseDownloadCard(
    ...
    // Provenance : la base radio installée a-t-elle été GÉNÉRÉE sur l'appareil plutôt que téléchargée ?
    // Tri-état : null = pas encore vérifié (ligne « actuellement téléchargée » masquée), true = générée, false = téléchargée.
    ...
    LaunchedEffect(isSyncing, refreshTrigger, txtSearching, txtUnknown, txtNoDb, txtInvalidDb, featureFlags) {
        withContext(Dispatchers.IO) {
            ...
            // Provenance déterminée AVANT le réseau : la ligne « actuellement téléchargée » ne s'affiche
            // qu'après cette vérification (sinon une base radio générée y clignote quelques secondes).
            isLocallyGenerated = LocalDbProvenance.readRadio(context).locallyBuilt
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(sizing.spacing(16.dp)), horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            // Ligne « actuellement téléchargée » : affichée UNIQUEMENT après vérification de provenance (== false),
            // masquée si la base radio a été générée sur l'appareil (== true) ou pas encore vérifiée (null).
            ...
            if (isSyncing) {
                ...
                val radioButtonLabel = when {
                    locallyGenerated -> stringResource(R.string.database_replace_generated_radio_with_online)
